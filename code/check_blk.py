...
def check_consistency(df, label):
    ...
    if not liab_parts.is_empty():
        ...
        print("WARNING: No fixed liability items found!")
    ...
    df_plot = df_plot.with_columns([
        ...
        (pl.col('TotalAssets') - (pl.col('TotalLiab') + pl.col('Equity'))).alias('Diff_API_Liab') # APIのTotalLiabを使った場合のDiff
    ])

    print(df_plot.select(['Date', 'TotalAssets', 'TotalLiabEquityCalc', 'Diff', 'Diff_API_Liab']))
    
    max_diff = df_plot['Diff'].abs().max()
    print(f"Max Difference: {max_diff}")
    
    if max_diff > 1.0: # 許容誤差
        print("!!! HUGE DIFFERENCE DETECTED !!!")
        bad_rows = df_plot.filter(pl.col('Diff').abs() > 1.0)
        for row in bad_rows.iter_rows(named=True):
            print(f"Date: {row['Date']}")
            print(f"  Total Assets: {row['TotalAssets']}")
            print(f"  Current Liab: {row['CurrLiab']}")
            print(f"  Fixed Liab:   {row['FixedLiab']}")
            print(f"  Equity:       {row['Equity']}")
            print(f"  Sum (L+E):    {row['TotalLiabEquityCalc']}")
            
            # 内訳確認
            date_val = row['Date']
            if not liab_parts.is_empty():
                parts = liab_parts.filter(pl.col('Date') == date_val)
                print("  Fixed Liab Parts:")
                print(parts)

print("Processing Annual Balance Sheet...")
check_consistency(extract_and_melt(ticker.balancesheet, target_bs), "Annual")

print("\nProcessing Quarterly Balance Sheet...")
check_consistency(extract_and_melt(ticker.quarterly_balancesheet, target_bs), "Quarterly")
